...
class Brickjump():
    ...
    def update(self):
        ...
        self.background_image_tinted.fill(TINTS[((max(4, self.score_value) - 4) // 100) % 5], special_flags=pygame.BLEND_RGBA_MULT)
        self.screen.blit(self.background_image_tinted, (0, 0))

        self.level.update(self.score_value)
        ...
        score_text = pygame.font.Font(font_path, 30).render(f'Score: {self.score_value}', True, (255, 255, 255))
        ...
        self.pps = self.score_value / max(1, self.elapsed_time)
        pps_text = pygame.font.Font(font_path, 30).render(f'Platforms per second: {self.pps:.2f}', True, (255, 255, 255))
